...
namespace BonusProje1
{
    public partial class FrmSinavNotlar : Form
    {
        ...
        private void FrmSinavNotlar_Load(object sender, EventArgs e)
        {
            ...
            da.Fill(dt);
            comboBox1.DisplayMember = "DERSAD"; // dsiplay -- gözüken bu olsun
            comboBox1.ValueMember = "DERSID";  // arka plandakı değeri ıd olacak
        ...
        private void BtnHesapla_Click(object sender, EventArgs e)
        {
            ...
            TxtOrtalama.Text = ortalama.ToString(); // sonuçta neden ondalıklı kısım yazmıyor?
            if (ortalama>=50)
            {
                TxtDurum.Text = "True";
            }
        ...
        private void BtnGuncelle_Click(object sender, EventArgs e) // dersıd yerine deradı yazsın >> ödev
        {
            ds.NotGuncelle(byte.Parse(comboBox1.SelectedValue.ToString()),int.Parse(TxtID.Text),byte.Parse(TxtSinav1.Text), byte.Parse(TxtSinav2.Text), byte.Parse(TxtSinav3.Text), byte.Parse(TxtProje.Text),
                decimal.Parse(TxtOrtalama.Text),bool.Parse(TxtDurum.Text),notid);
        }
    }
}
